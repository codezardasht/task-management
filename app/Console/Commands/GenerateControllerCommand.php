...
class GenerateControllerCommand extends GeneratorCommand
{
     ...
     * @var string
     */
    protected $signature = 'technobase:controller
                            {name : The name of the controler.}
                            {--crud-name= : The name of the Crud.}
                            {--crud-model= : The name of the Crud.}
     ...
     * @return string
     */
    protected function buildClass($name , $request = 'request')
    {
        $model_crud = explode(',' , $this->option('crud-model'));
        $modelName = strtolower($this->option('model-name'));
        $new_model = "";

        if(count($model_crud)> 0 )
        {
            foreach ($model_crud as $crud) {
                $new_model .= "$$modelName->$crud = $$request->$crud;\n";
            }
        }
        ...
        $crudNameSingular = \Illuminate\Support\Str::singular($crudName);
...
EOD;
        ...
        return $this->replaceNamespace($stub, $name)
            ...
            ->replaceWhereNewModel($stub, $new_model)
    ...
    protected function replaceWhereNewModel(&$stub, $crudNameSingular)
    {

        $stub = str_replace(
            '{{NewModel}}', $crudNameSingular, $stub
        );
